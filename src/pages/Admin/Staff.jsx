...
const Staff = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingStaff) {
        await updateAdminStaff(editingStaff._id, formData);
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div
        className={`p-4 rounded-xl border flex justify-between items-center ${isDark ? "bg-slate-900 border-slate-800" : "bg-white border-gray-200"}`}
      >
        <h2
        ...
        <button
          onClick={() => openModal()}
          className={`flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white rounded-lg shadow-lg transition-all active:scale-95 ${
            isDark
      ...
      {/* Table */}
      <div
        className={`rounded-xl border overflow-hidden ${isDark ? "bg-slate-900 border-slate-800" : "bg-white border-gray-200"}`}
      >
        <table className="w-full text-left">
          <thead className={isDark ? "bg-slate-800/50" : "bg-gray-50"}>
            <tr>
              {["Name", "Email", "Role", "Status", "Actions"].map((h) => (
                <th
                  key={h}
                  className={`px-6 py-3 text-xs font-semibold uppercase ${isDark ? "text-slate-400" : "text-gray-500"}`}
                >
                  {h}
                </th>
              ))}
          ...
          <tbody
            className={
              isDark ? "divide-y divide-slate-800" : "divide-y divide-gray-100"
            }
          >
