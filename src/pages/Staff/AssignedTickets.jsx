...
//   return (
// <div className="flex h-[calc(100vh-120px)] border rounded-xl overflow-hidden">
...
import api from "../../api/axios";
import { socket } from "../../socket/socket";
import { Search, User, MessageSquare, Clock, CheckCircle, AlertCircle, ChevronLeft, ChevronRight, Phone, Mail, Calendar, Filter, X } from "lucide-react";
...
const AssignedTickets = () => {
  ...
  const getStatusStyles = (status) => {
    const base = "text-xs px-2.5 py-1 rounded-full font-medium inline-flex items-center gap-1.5";
    
    if (isDark) {
      switch(status?.toLowerCase()) {
        case 'open': return `${base} bg-blue-500/20 text-blue-300`;
        case 'pending': return `${base} bg-amber-500/20 text-amber-300`;
        case 'resolved': return `${base} bg-emerald-500/20 text-emerald-300`;
        case 'closed': return `${base} bg-gray-800 text-gray-300`;
        default: return `${base} bg-gray-800 text-gray-300`;
      }
    } else {
      switch(status?.toLowerCase()) {
        case 'open': return `${base} bg-blue-100 text-blue-700`;
  ...
  const statusFilters = [
    ...
    { key: "open", label: "Open", count: rooms.filter(r => r.status === "open").length },
    { key: "pending", label: "Pending", count: rooms.filter(r => r.status === "pending").length },
    { key: "resolved", label: "Resolved", count: rooms.filter(r => r.status === "resolved").length },
  ];
  ...
  return (
    <div className={`relative h-[calc(100vh-120px)] overflow-hidden ${isDark ? 'bg-gray-900' : 'bg-white'}`}>
          ...
          <div className="flex-1 overflow-y-auto">
            {filteredRooms.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full p-4">
                ...
                <p className={`text-center text-base ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-1.5 mb-1">
                          <span className={getStatusStyles(room.status)}>
                            ...
                            <span className="text-xs capitalize">{room.status || 'open'}</span>
                          </span>
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-4">
              <div className={`max-w-sm p-6 rounded-lg ${isDark ? 'bg-gray-800' : 'bg-gray-50'}`}>
                <div className={`w-16 h-16 rounded-lg flex items-center justify-center mx-auto mb-4 ${
                  ...
                  <MessageSquare className={`w-8 h-8 ${isDark ? 'text-gray-500' : 'text-gray-400'}`} />
                </div>
                ...
                <div className="flex items-center justify-center gap-3 text-xs">
                  <div className={`flex items-center gap-1 px-2 py-1 rounded ${isDark ? 'bg-gray-700 text-gray-300' : 'bg-gray-100 text-gray-700'}`}>
                    <div className="w-1.5 h-1.5 rounded-full bg-green-500"></div>
                    <span>Open: {rooms.filter(r => r.status === 'open').length}</span>
                  </div>
                  <div className={`flex items-center gap-1 px-2 py-1 rounded ${isDark ? 'bg-gray-700 text-gray-300' : 'bg-gray-100 text-gray-700'}`}>
                    <div className="w-1.5 h-1.5 rounded-full bg-amber-500"></div>
                    <span>Pending: {rooms.filter(r => r.status === 'pending').length}</span>
                  </div>
                </div>
              </div>
            </div>
